            ...
            {solutions.map((item, index) => {
              ...
              return (
                <div key={item.title} className="rounded-[30px] border border-white/10 bg-white/[0.06] p-7 shadow-panel backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:border-[#ffbc92]/35">
                  ...
                  <div className="mt-4 rounded-2xl bg-white/[0.08] px-4 py-3 text-sm text-[#ffd4bc]">{item.suggestion}</div>
                </div>
              );
            })}
          </div>
      ...
      <section className="bg-[rgba(24,11,23,0.68)] text-white">
        <div className="mx-auto max-w-7xl px-4 py-20 md:px-8 lg:px-10">
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            <Link href="/lien-he" className="rounded-full border border-white/20 bg-white/[0.08] px-6 py-4 font-semibold text-white">Đi tới Liên hệ</Link>
          </div>
        </div>
      </section>
    </main>
